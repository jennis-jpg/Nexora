...
from config import config_status, get_settings, load_and_validate
...
from routes.coastguard import router as coastguard_router
from routes.incidents import router as incidents_router
from routes.predict import router as predict_router
from routes.report import router as report_router
from routes.sea_level import router as sea_level_router
from routes.sos import router as sos_router
from routes.voice import router as voice_router
from routes.weather import router as weather_router
from services.scheduler import shutdown_scheduler, start_scheduler

# Load and validate environment variables (reads backend/.env if present)
load_and_validate()
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=get_settings().cors_origins,   # configure via CORS_ORIGINS env var
    allow_methods=["*"],
    allow_headers=["*"],
)
...
app.include_router(coastguard_router)
app.include_router(sea_level_router)
...
@app.get("/config/status", tags=["System"], summary="Non-sensitive service configuration status")
async def get_config_status() -> dict:
    """Returns which backend services are configured.

    Safe to expose publicly — never includes secret values, only whether each
    service is configured.  Used by the frontend Settings panel in dev mode.
    """
