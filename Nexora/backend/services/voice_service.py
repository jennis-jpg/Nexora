...
both directly and as the caption for the pre-recorded Tamil MP3s), and
resolves which audio asset the frontend should play for a given risk
level + language.

Primary language: Tamil (pre-recorded MP3s: tamil_safe/warning/danger.mp3).
Secondary language: Kannada (pre-recorded MP3s: kannada_safe/warning/danger.mp3).
Fallback: English uses the browser Web Speech API / OS TTS ("system_voice").

Place MP3 files in backend/sound/ — the /audio/* static mount serves
whatever files are present in that directory; no filenames are hardcoded here.
...
Tamil/Kannada MP3s served from /audio/*, or the Web Speech API / OS TTS
for "system_voice").
"""
...
_LANGUAGE_TAMIL   = {"tamil", "ta", "tam", "தமிழ்"}
...
_TAMIL_AUDIO = {
    "low": "/audio/tamil_safe.mp3",
    "medium": "/audio/tamil_warning.mp3",
    "high": "/audio/tamil_danger.mp3",
}
...
def resolve_audio_url(risk_level: str, language: str) -> Optional[str]:
    ...
    if language in _LANGUAGE_TAMIL:
        return _TAMIL_AUDIO[risk_level]
