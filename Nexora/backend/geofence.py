...
RISK_HIGH_MAX_MIN = 15     # < 15 min  -> High
RISK_MEDIUM_MAX_MIN = 29   # 15-29 min -> Medium
                            # > 29 min  -> Low

# ---------------------------------------------------------------------------
# Valid operational regions
#
# Boats outside both regions are still processed (boundary math runs fine),
# but the response includes region="Unknown" so the frontend can flag the
# anomaly, and a WARNING is logged at the service layer.
#
# Bounds are intentionally wider than the minimum to accommodate boats
# approaching the region edges and GPS jitter near coastlines.
#
#   Palk Strait & Gulf of Mannar  — primary area; IMBL points B1-B6 lie here
#   Karnataka Coast (Arabian Sea) — secondary area; west-coast fishing grounds
# ---------------------------------------------------------------------------
VALID_REGIONS: list[dict] = [
    {
        "name":    "Palk Strait & Gulf of Mannar",
        "lat_min": 8.5,  "lat_max": 10.5,
        "lon_min": 78.5, "lon_max": 81.0,
    },
    {
        "name":    "Karnataka Coast (Arabian Sea)",
        "lat_min": 12.0, "lat_max": 15.0,
        "lon_min": 73.5, "lon_max": 76.0,
    },
]


def get_region(lat: float, lon: float) -> str:
    """Return the name of the valid operational region the position falls in.

    Checks each region in ``VALID_REGIONS`` in order; the first match wins.
    Returns ``"Unknown"`` if the position is outside all defined regions.
    """
...
def is_in_valid_region(lat: float, lon: float) -> tuple:
    """Return ``(True, region_name)`` if the coordinates fall within a known
    operational region, or ``(False, None)`` otherwise.

    The tuple is typed as ``tuple`` for Python < 3.9 compatibility (rather
    than ``tuple[bool, str | None]``).
    """
    name = get_region(lat, lon)
    if name == "Unknown":
        return False, None
    return True, name
