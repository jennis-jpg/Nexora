...
def render_sos_button(*, key: str = "sos_btn") -> bool:
    ...
    if triggered:
        ...
        try:
            from api_client import call_sos
            call_sos(
                boat_id=st.session_state.get("vessel_data", {}).get("boat_id", "unknown"),
                lat=float(st.session_state.get("vessel_data", {}).get("latitude", 0.0)),
                lon=float(st.session_state.get("vessel_data", {}).get("longitude", 0.0)),
            )
        except Exception:
            pass  # SOS is recorded locally; backend call is best-effort
        st.markdown(
            f"""
