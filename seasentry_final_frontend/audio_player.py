...
def render_chained_alert_audio(
    ...
    <div style="display:flex; align-items:center; gap:10px; font-family:sans-serif;">
        ...
        <span id="status-{key}" style="font-size:0.72rem; color:rgba(255,255,255,0.55);">
            Attempting autoplay…
        </span>
    </div>
    <script>
    (function() {{
        ...
        function playAudio(url) {{
            return new Promise((resolve) => {{
                ...
                a.onended = resolve;
                a.onerror = resolve;
                a.play().catch(() => resolve());  // if blocked, just move to next stage
            }});
        }}
        ...
        // Optimistic autoplay attempt -- browsers may block this depending
        // on their user-gesture policy. The button above always works as a
        // guaranteed manual fallback regardless.
        playSequence().catch(() => {{
            statusEl.textContent = "Autoplay blocked — click Play Alert";
        }});
    }})();
    </script>
    """
