...
def render_prediction_controls() -> None:
    ...
    def _reset() -> None:
        ...
        st.session_state.pc_heading = DEMO_VESSEL["heading_deg"]
        st.session_state.pc_engine = True
        st.session_state.pc_language = "kannada"
        st.session_state.pc_submitted = False
    ...
    language_val = st.selectbox(
        ...
        format_func=lambda v: {"kannada": "ಕನ್ನಡ · Kannada", "english": "English"}[v],
        key="pc_language",
        help=(
            "Kannada plays a recorded human voice clip. English has no "
            "recording yet, so it's spoken via your browser's built-in "
            "text-to-speech instead."
        ),
    )
