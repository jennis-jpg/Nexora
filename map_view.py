    ...
    <div class="ss-boat-wrap">
      ...
      <div class="ss-boat-bob">
        <div class="ss-boat-rot" style="transform:translate(-50%,-50%) rotate(${headingDeg}deg);position:absolute;left:50%;top:50%;">
          <svg class="ss-boat-svg" width="22" height="22" viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="9" fill="${b.riskColor}" stroke="#fff" stroke-width="2"/>
    ...
    st.markdown(
        ...
        <div class="map-legend">
            <div class="legend-item">
                <span class="legend-swatch" style="background:{COLORS['green']};"></span> Boat Position
            </div>
            <div class="legend-item">
                <span class="legend-swatch" style="background:{COLORS['sky']};"></span> Route Before Border
            </div>
            <div class="legend-item">
                <span class="legend-swatch" style="background:{COLORS['red']};"></span> Route After Crossing
            </div>
            <div class="legend-item">
                <span class="legend-swatch" style="background:{COLORS['red']}; border-radius:50%;"></span> Crossing Marker
            </div>
            <div class="legend-item">
                <span class="legend-swatch" style="background:{COLORS['red']}; opacity:0.3;"></span> Risk Circle
            </div>
            <div class="legend-item">
...
def render_dashboard_layout(vessel: dict[str, Any]) -> None:
    ...
    # FIXED: the map used to always render from `vessel` (last real /predict
    # result, or the DEMO_VESSEL placeholder on first load), which only
    # changes after clicking "Predict Drift" -- so as you typed a new
    # lat/lon/heading into the controls below, the map wouldn't move until
    # your NEXT click, always showing one step behind what you'd just typed.
    #
    # Streamlit updates st.session_state for keyed widgets (pc_lat, pc_lon,
    # etc.) at the START of every rerun, before any of our code runs -- so
    # we can read the vessel's CURRENT live input values here, before the
    # map is drawn, and before render_prediction_controls() (which owns
    # those widgets) even runs later in this same function. This makes the
    # map track the input fields in real time.
    ...
    # Drop any predicted-position/risk data left over from a PREVIOUS
    # prediction -- if we kept it, the map would show an old drift arrow
    # and risk marker that no longer corresponds to the freshly-typed
    # inputs above, which would be its own confusing bug. The map falls
    # back to its own placeholder trajectory projection until a fresh
    # prediction is actually run.
    preview_vessel.pop("pred_latitude", None)
    preview_vessel.pop("pred_longitude", None)

    render_top_kpi_row(vessel)  # KPI/alert panels intentionally still show the last REAL prediction
